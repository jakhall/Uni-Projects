...
public class HomeModel extends BaseModel {
	...
	public void addUserDocument() {
			...
			doc.setImageLocation("default");
			doc.setTextLocation("default");
	...
	public ArrayList<Document> getSuggestions(Profile user, int position, int range){
		...
		VectorSpaceModel vsm = theSession.getVSM();
		...
		ArrayList<Integer> viewedDocs = profileDB.getViewedDocuments(user.getUserID());
		double similarity;
		int index;
		...
		for(Document doc : allDocuments) {
			...
			if(index < 0) {
				...
				if(similarity > 0 && !viewedDocs.contains(doc.getID())) {
	...
	public ArrayList<Document> getRecentList(Profile user){
		String sql = "SELECT UserID FROM OBSERVER_Table WHERE ObserverID=" + user.getUserID();
		...
		Comparator<Document> c = new Comparator<Document>() {
		    ...
		    public int compare(Document a, Document b) {
		    	...
		    	Date dateB = null;
		    		try {
						dateA = format.parse(a.getLastUpdated());
						dateB = format.parse(b.getLastUpdated());
