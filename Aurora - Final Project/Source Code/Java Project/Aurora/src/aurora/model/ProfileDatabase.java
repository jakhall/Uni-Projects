...
public class ProfileDatabase {
	...
	public Profile getLogin(String u, String p){
		...
		sql = "Select * From USER_Table WHERE LOWER(Username)= LOWER('" + u + "') AND Password='" + p + "'";
		ResultSet rs = queryDatabase(sql);
		ArrayList<Profile> profiles = createProfiles(rs);
		Profile user = null;
		if(profiles.size() > 0) {
	...
	public ArrayList<Document> createDocuments(ResultSet rs){
		...
		try {
			while(rs.next()) {
				...
				doc.setImage(getDocumentImage(rs.getString(6)));
	...
	public byte[] getImage(String location) {
		...
		if(!location.startsWith("\\")) {
			try {
				return IOUtils.toByteArray(ProfileDatabase.class.getResourceAsStream("/default_data/profile_image/default_" + location + ".png"));
	...
	public byte[] getDocumentImage(String location) {
		
		File imageFile = null;
		
		if(!location.startsWith("\\")) {
			try {
				return IOUtils.toByteArray(ProfileDatabase.class.getResourceAsStream("/default_data/1.png"));
			} catch (IOException e1) {
				e1.printStackTrace();
			}
		}
